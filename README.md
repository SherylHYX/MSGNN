...
- The original paper has one typo in the range of $\Theta^{(q_0)},$ which should indeed be $[-\pi, \pi]$ instead of $[0, \pi]$. If the goal of choosing $q$ is to make those maps of angles clear enough without any ambiguity (the full circle in our current setting is OK, but not ideal, since values very close to $\pi$ are treated similarly to values very close to $-\pi$ due to the circular property of angles), an actual recommended $q$ value should be half of the current $q_0$. This typo only affects real-world dataset results, since for synthetic datasets, the default $q$ is upper bounded by 0.25 (which is already the half of this $q_0$ value).
